...
public class GovernanceGraph extends JPanel {
	...
	private BasicStroke expertStroke = new BasicStroke(2.0F, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0F, dashes, 5.0F);
	...
	private PropertyResourceBundle govRes;

	private boolean hasExpertValues = true;
	private int[] expertValues;

	public GovernanceGraph(int[] expertVals) {	
		if (expertVals == null) {
	...
	public void setLanguage() {
		if (MIDAS.LANGUAGE.equals("EN")) {
			govRes = (PropertyResourceBundle) ResourceBundle.getBundle("Graphs", new Locale("en"));
		}
		else if (MIDAS.LANGUAGE.equals("ES")) {
			govRes = (PropertyResourceBundle) ResourceBundle.getBundle("Graphs", new Locale("es"));
		}

		title = govRes.getString("GOV_TITLE");
		yTitle[0] = govRes.getString("GOV_YTITLE1");
		yTitle[1] = govRes.getString("GOV_YTITLE2");

		xTitle = govRes.getString("CDF_XTITLE");
		yValues[0] = govRes.getString("CDF_YVAL1");
		yValues[1] = govRes.getString("CDF_YVAL2");
		yValues[2] = govRes.getString("CDF_YVAL3");
		yValues[3] = govRes.getString("CDF_YVAL4");
		yValues[4] = govRes.getString("CDF_YVAL5");
		xValues[0] = govRes.getString("CDF_XVAL1");
		xValues[1] = govRes.getString("CDF_XVAL2");
		lineNames[0] = govRes.getString("CDF_USER_PRES");
		lineNames[1] = govRes.getString("CDF_USER_FUTURE");
		lineNames[2] = govRes.getString("CDF_EXP_PRES");
		lineNames[3] = govRes.getString("CDF_EXP_F_OPT");
		lineNames[4] = govRes.getString("CDF_EXP_F_WRST");
	}
	...
	public void calculatePts() {
		 ...
		 */
		presentRatio = startY + yAxisLength / 2 - midasCalc.calcUserGovIndex() * yAxisLength / 2;
		futureRatio = startY + yAxisLength / 2 - midasCalc.calcGovIndexFuture(MIDAS.govCDF[MIDAS.mmaNum][5]) * yAxisLength / 2;
	...
	public void paintComponent(Graphics g) {		
		...
		roundedRectangle = new RoundRectangle2D.Float(30, 40, 
				(int)(MIDAS.OUTCOMERES.getWidth() - 60), (int)(MIDAS.OUTCOMERES.getHeight() - 80), 15, 15);
		g2d.fill(roundedRectangle); 
		...
		for (int i = 0; i < yValues.length; i++) {
			...
			g2d.setColor(Color.black);
			g2d.fillRect(axisStartX - legendBarWidth - axisWidth, startY + barHeight * i, legendBarWidth, barHeight);
