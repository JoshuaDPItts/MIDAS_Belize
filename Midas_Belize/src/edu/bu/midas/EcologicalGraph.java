...
public class EcologicalGraph extends JPanel {
	...
	public EcologicalGraph(int[] expertVals) {	
		if (expertVals == null) {
	...
	{	
		if (MIDAS.LANGUAGE.equals("EN")) {
			ecoRes = (PropertyResourceBundle) ResourceBundle.getBundle("Graphs", new Locale("en"));
		}
		else if (MIDAS.LANGUAGE.equals("ES")) {
			ecoRes = (PropertyResourceBundle) ResourceBundle.getBundle("Graphs", new Locale("es"));
		}

		title = ecoRes.getString("ECO_TITLE");
		yTitle[0] = ecoRes.getString("ECO_YTITLE1");
		yTitle[1] = ecoRes.getString("ECO_YTITLE2");

		xTitle = ecoRes.getString("CDF_XTITLE");
		yValues[0] = ecoRes.getString("CDF_YVAL1");
		yValues[1] = ecoRes.getString("CDF_YVAL2");
		yValues[2] = ecoRes.getString("CDF_YVAL3");
		yValues[3] = ecoRes.getString("CDF_YVAL4");
		yValues[4] = ecoRes.getString("CDF_YVAL5");
		xValues[0] = ecoRes.getString("CDF_XVAL1");
		xValues[1] = ecoRes.getString("CDF_XVAL2");
		lineNames[0] = ecoRes.getString("CDF_USER_PRES");
		lineNames[1] = ecoRes.getString("CDF_USER_FUTURE");
		lineNames[2] = ecoRes.getString("CDF_EXP_PRES");
		lineNames[3] = ecoRes.getString("CDF_EXP_F_OPT");
		lineNames[4] = ecoRes.getString("CDF_EXP_F_WRST");
	}
	...
	public void calculatePts() {
		 ...
		 */
		presentRatio = startY + yAxisLength / 2 - midasCalc.calcUserEcoIndex() * yAxisLength / 2;
		futureRatio = startY + yAxisLength / 2 - midasCalc.calcEcoIndexFuture(MIDAS.ecoCDF[MIDAS.mmaNum][5]) * yAxisLength / 2;
	...
	public void paintComponent(Graphics g) {
		...
		//g2d.drawLine(width/2, 0, width/2, 100);	//for centering purposes
